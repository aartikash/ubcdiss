...
\begin{enumerate}
	...
	\ac{APS} is a medical system that consists of a standard feed-forward network with no exotic\karthik{Awkward phrasing} features such as normalization 
	unlike \ac{ACAS-Xu}. 
	To accommodate normalization, we have to translate the intuition behind\karthik{Seems strange that you'd translate the intuition} it to a \ac{MILP} model which is not straightforward.  
	...
	Hence, we need a technique that is scalable to bigger systems. \karthik{Why don't we say more complex, as complexity includes size as we defined?} 
...
DNN architecture is quite complex as explained in ~\ref{dnncomplexity}, and researchers have used experimental means to understand the security scenarios in the DNN \karthik{Citation?}. 
Their \karthik{Who ? The researchers' ?}  mathematical structure contains multiple layers, which is the first layer of complexity \karthik{is this a DNN layer?}, and the non-linearity introduced due to the activation functions within the network add another layer of complexity for modeling in \ac{MILP}  that we elaborate below. \karthik{You overload the word layer to mean both DNN layers and layers of complexity. This is confusing !} 
...
\begin{enumerate}
	...
	\item Non-linear terms are not allowed in \ac{MILP} modeling \karthik{Citation}. Hence, the main challenge is how to model the activation functions since they are non-linear. 
\end{enumerate}
...
However, our requirement is different in the traditional sense of using minimization and maximization, which is usually used in terms of resource utilization\karthik{I don't understand this sentence} ~\cite{7214162}.  
...
\begin{enumerate}
	...
	This happens if the \ac{MIP} is infeasible, \karthik{Did we define this earlier ? Refer to it.} and we need to redefine our cost function. 
...
We demonstrate both the problems in our experimental evaluation \karthik{Sorry, did you find the problems or did you demonstrate that they don't arise? How can  you demonstrate a problem ?}, 
