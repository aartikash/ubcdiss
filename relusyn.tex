...
The equation in this rewritten form is linear, and it does model ReLU functionality  correctly \karthik{Seems like a strange thing to say - did you mean accurately?} as $x$ will always be non-negative. 
...
Extending this scalar example to the DNN case leads to a \ac{MILP} model of the form: \karthik{Should there be an equation below?}
...
These cost functions are different for different applications and depends on our goal; for an attack synthesis application minimization of input deviations is the goal whereas for a resource utilization \karthik{Sentence is incomplete}.
