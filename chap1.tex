...
We demonstrate a vulnerability in these systems \karthik{many vulnerabilities, correct?} by using our technique to synthesize a new class of attacks for DNN based controllers that perturbs critical inputs to produce changes in output. 
...
To mount a successful \ac{RFDIA} attack on a DNN based controller, the attacker needs to distort an input such \karthik{should we say maximally distort?} that the effects of its perturbation reach the final layer, without triggering any alarms. 
...
\begin{enumerate}
	...
	\item \textbf{Perturbations for each critical input}: The smallest perturbations that will trigger the desired change in output. \karthik{Say why it should be smallest here}
\end{enumerate}
...
\begin{enumerate}
	...
	\item We demonstrate the affects of introducing normalization layers in \ac{DNN} by showing they make \ac{DNN}s more robust against \ac{RFDIA}. \karthik{I think it's a bit of a stretch to say we demonstrate this as we didn't do a targeted experiment with normalization. Perhaps tone it down and say we 'hypothesize' instead.}
\end{enumerate}
