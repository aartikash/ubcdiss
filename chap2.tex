...
\begin{enumerate}
	...
	build the \ac{DNN} architectures \karthik{Did you expand this?}, e.g., an architect can use different types of \ac{ML} models such as decision trees, or \ac{DNN} to implement the system.
...
The physical and virtual contexts can either be humans in the loop or some automated mechanism or nothing at all. \karthik{How can it be nothing?}
...
In closed-loop systems, there are no regular human interventions \karthik{What is 'regular'?} 
as shown in Figure 2.3 (below diagram). 
In the latter case, understanding different types of security attacks becomes a necessity. \karthik{Why?}
...
inputs under a constrained setting that can cause ripples. \karthik{This is a controversial statement, and quite unnecessary for your work.}
...
A \ac{NN}\karthik{expand} consists of input, output and hidden layers. 
...
This system is considerably bigger in size and complexity as compared to \ac{APS}. \karthik{What about the 3rd system ?}
...
\karthik{What other models ? Also, did you mean 'involve' feature engineering ?}
Feature engineering is the automatic means to extract features automatically from raw data. \karthik{What's the relationship bet. feature engg. and normalization?}
...
which we have to consider in our abstraction modeling. \karthik{I guess you meant the abstraction layer, not the systems}

However,  there are certain\karthik{how many?}  problems associated with \ac{DNN} based controller models. 
Since these systems are complex as in \ac{ACAS-Xu}, the inner workings and the behavior is not completely understood \karthik{Really ? By whom ?}. 
The complexity depends on  two main factors: the size and the architecture.
Adding a new hidden layer to the network increases the size that has  significant impact on the systems. \karthik{increases the complexity?}  
...
\karthik{Sorry, but the above paragraph is incoherent}
...
We present a general structure that is required for building a \ac{LP} \ac{MILP} models. \karthik{What's the connection here?}
...
\karthik{You should replace the above paragraph with a bulleted list}
