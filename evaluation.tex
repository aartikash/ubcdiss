...
Table \ref{tab:table1} presents the architectures of the three \ac{DNN}s used in the \ac{APS}, \ac{HCAS}, and \ac{ACAS-Xu}, in increasing order of complexity. 
...
We wrote a Python script to synthesize \ac{MILP} models from \ac{DNN}s and used it as our Gurobi input. 

As explained in Chapter \ref{relusyn}, our tool \tool, converts the \ac{DNN} to \ac{MILP} format and provides the critical inputs and the perturbations. 
We ran our experiments on server with a 2.40 GHz (10 MB L3 cache) Intel Xeon E5-2407 v2 processor
...
The \ac{APS} takes 74 inputs ($37$ pairs of insulin and glucose values) gathered every $5$ minutes. 
It then predicts the next insulin value to be injected, which is $5$ minutes later than the last calculated output by the network. 
%
%Attacks in the APS system.
Using \tool, we can successfully synthesize attacks in less than a minute, making it cost effective for an attacker.
...
For each perturbation scenario, we evaluate our attack efficacy for a range of different outcomes. 
 In \ac{APS}, if a normal output value is $x$; we consider $n$ different output ranges as shown in Table ~\ref{APS} and \ref{criticalinputs}, each of which administers too much insulin, differing only in how much additional insulin is administered.	
 We then exhaustively test each unique attack in the perturbation scenario until we obtain the smallest one.
 For example as shown in Table ~\ref{APS}, if the perturbation scenario is "Attack only a single input, change its value by $<= +/- 5$ and the objective is input perturbation minimization" then, given 74 inputs, we run 74 tests for each target output range until we find a feasible input. 
 ...
 We iterate over the 74 different inputs, perturbing each by a random amount in the range ($x$ - $5$) $<=$ ($x$ + $5$) until we find the one with the smallest perturbation. 
 Finally, the inputs that on small perturbations lead to output deviations  become our critical inputs set as shown in Table ~\ref{criticalinputs} for one specific case. 
...
\begin{enumerate}
	...
	\item  We show that it takes less than a second for \ac{APS} to identify the critical inputs and find the minimum perturbations for all the attacks.
...
Each network has the architecture described Table ~\ref{tab:table1} with $5$ inputs, $6$ hidden layers and $5$ outputs.  
...
\ac{ACAS-Xu}'s goal is to avoid crashes of the ownship with the intruder; if the two are getting too close, an action is taken that moves the two apart. 
The output in the \ac{DNN} corresponds to the action associated with the minimal score. 
...
\ac{HCAS} functions similarly to \ac{ACAS-Xu} apart from the architectural differences as stated in Table 5.1. 
...
\subsubsection{Random Ordering Attack}
...
The orderings of other outputs is of no concern to the attacker so long as $COC$ has the least score. 
...
For this to happen the score for $2$ has to be smallest of all other outputs. 
...
they have to state four constraints in this case, $value(2)$ $<$ $(all$ $other$ $values)$.
...
\begin{enumerate}
	...
	\item When we tried finding the smallest perturbation, several experiments timed out but finding a perturbation within possible inputs was successful. 
\end{enumerate}	
...
\begin{enumerate}
	...
	\item We conducted ~30 \ac{HCAS} random attacks out of which ~20 were successful, and the maximum amount of time taken by the attacks was 41 seconds.
	\item We conducted ~7 targeted attacks for \ac{HCAS} out of which 1 attack was successful.
	\item We conducted ~140 \ac{ACAS-Xu} random attacks out of which ~32 were successful, and the maximum amount of time taken by the attacks was 7228 seconds.
	\item We conducted ~16 targeted attacks for \ac{HCAS}, out of which 2 attacks were successful.
\end{enumerate}
