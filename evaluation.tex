...
Our tool currently supports TensorFlow and PyTorch   \ac{DNN} formats. \karthik{Add citations}
We wrote a Python script\karthik{Please check} to synthesize \ac{MILP} models from \ac{DNN}s, and used it as our Gurobi interface. 
...
We ran our experiments in Gurobi's default mode that spawns multiple threads for solving a model, using an 8-core system. \karthik{Provide more details about system}
...
As mentioned before\karthik{Add section ref}, \ac{APS} takes 74 inputs ($37$ pairs of insulin and glucose values) gathered every $5$ minutes. 
...
Using \tool, we can successfully synthesize attacks in less than a minute, which is reasonable for an attacker to mount attacks in real-time on the system\karthik{please check}.
...
For each perturbation scenario, we evaluate our attack efficacy for a range of different outcomes. 
 ...
 We randomly\karthik{Do you use a random number generator ? I'm confused.} chose $5$ in our evaluation despite the units of insulin and glucose units being different,  because we have constraints in the model that add the maximum and minimum values of insulin and glucose, which if $<$ $5$ and $>$ $-5$  respectively override $5$. \karthik{Sorry, I don't understand this}
 ...
 We iterate over the 74 different inputs, perturbing each by a random amount in the range ($x$ - $5$) $<=$ ($x$ + $5$) till we find the one with the smallest perturbation. \karthik{I guess you have to try them all to find the smallest, so I don't understand why you say till ?}
 Finally, the inputs that on smallest perturbations lead to output deviations \karthik{Is it 1 input or many ? The previous sentence makes it sound like it's unique} becomes our critical inputs set as shown in Table ~\ref{criticalinputs} for one specific case. 
...
To locate the critical inputs we first perturb the inputs, one at a time, to induce perturbations in the outputs \karthik{Didn't you just say this ?} as  shown in Table ~\ref{summary} and Table ~\ref{APS}. 
...
\section{Summary}
\begin{enumerate}
	\item Overall, we conducted ~19,000 attacks for \ac{APS} out of which ~7200 attacks were successful, and took less than 1 second \karthik{Did you mean per attack ? Or total time ?}. 
	...
	\item We conducted ~140 \ac{ACAS-Xu} random attacks out of which ~32 \karthik{~ means approximate - don't use it for exact values ! Or is 32 approximate ? Same for many of the values above. } were successful, and the maximum amount taken by the attacks was 7228 seconds.
