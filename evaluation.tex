...
We evaluate \tool with three networks: \ac{APS}, \ac{HCAS}, and \ac{ACAS-Xu}.
The reason we chose these three systems is because they are in increasing order of complexity, are from two different application domains to show generality. 
Table \ref{tab:table1} shows the sizes of the different networks we have used to evaluate our technique. They are defined by the number of inputs, layers, neurons per layer, and the outputs. 
